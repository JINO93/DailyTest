...
import com.example.administrator.test.view.pickerview.adapter.WheelAdapter;
...
public abstract class BasePickerView<T> extends View {
  ...
  private boolean isTouching = false; //手指按下
  ...
  private void init(AttributeSet attrs) {
    ...
    if (mItemSize == 0) mItemSize = ViewUtils.dipToPx(getContext(), sDefaultItemSize);
  }
  ...
  public void autoScrollFast(final int position, long duration) {
    float speed = ViewUtils.dipToPx(getContext(), 0.6f);
  ...
  public void setItemSize(int itemSize) {
    mItemSize = ViewUtils.dipToPx(getContext(), itemSize <= 0 ? sDefaultItemSize : itemSize);
  }
