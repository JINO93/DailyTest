package com.example.administrator.test.view.pickerview;
...
public class PickerView<T> extends BasePickerView<T> {
  ...
  private void init(AttributeSet attrs) {
    ...
    if (mOutTextSize <= 0) {
      mOutTextSize = ViewUtils.dipToPx(getContext(), sOutTextSize);
    }
    if (mCenterTextSize <= 0) {
      mCenterTextSize = ViewUtils.dipToPx(getContext(), sCenterTextSize);
    }
  ...
  public void setTextSize(int minText, int maxText) {
    mOutTextSize = ViewUtils.dipToPx(getContext(), minText);
    mCenterTextSize = ViewUtils.dipToPx(getContext(), maxText);
    invalidate();
  }
  ...
  public void drawItem(Canvas canvas, T data, int position, int relative, float moveLength,
    ...
    // 不换行
    StaticLayout layout =
      new StaticLayout(text, 0, text.length(), mPaint, ViewUtils.dipToPx(getContext(), 1000), mAlignment,
  ...
  private void computeColor(int relative, int itemSize, float moveLength) {
    ...
    if (relative == -1 || relative == 1) { // 上一个或下一个
      ...
      if ((relative == -1 && moveLength < 0) || (relative == 1 && moveLength > 0)) {
        ...
        color = computeGradientColor(mCenterColor, mOutColor, rate);
      }
